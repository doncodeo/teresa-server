...
const getAllUsers = asyncHandler(async (req, res) => {
  // Populate details from familyIds (array of family references)
  const users = await userData.find().populate({
    path: 'familyIds',
    select: 'name description', // Specify the fields to retrieve from the Family model
  });
...
const getUserById = asyncHandler(async (req, res) => {
  ...
  // Populate family details for the user's familyIds
  const user = await userData.findById(userId).populate({
    path: 'familyIds',
    select: 'name description createdBy', // Specify the fields to retrieve from the Family model
  });
